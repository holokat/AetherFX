...
- Speed is a control like the others, except that what it binds to is the document: {"node": "$effect", "parameter": "time_scale", "op": "multiply"}, min 0.25, max 4, default 1, step 0.05, unit "x". It changes how fast the effect plays (effect_time = wall_time * time_scale), not what it simulates, so the timeline, the keyframes and the phases stay exactly where you put them. Set the effect's own `time_scale` with set_effect_property when an effect should simply be quicker or slower than you authored it; ship the Speed control so a game can tune it per cast.
...
- Lightning STRIKE (not a neon tube): a beam with the defaults is a constant-width glowing line - the strike parameters are what make it read as lightning. Main bolt: width 0.16-0.24, segments 18-24, `detail: 3` (fractal midpoint jaggedness; vertices become segments * 2^detail + 1, so keep segments modest), noise_amplitude 0.3-0.45, jitter_rate 26-34, `width_profile: bulge` (hot shoulder near the sky, thin channel to the ground; `taper_end` for a fork, `taper_both` for an arc that hangs between two points), `width_variance: 0.3-0.4`, `intensity_noise: 0.3-0.45`, `flicker: 0.35-0.5` with `flicker_frequency: 40-55`, `afterglow: 0.07-0.09` (the previous path lingers as a fading ghost so the re-roll strobes instead of jumping), `branching: 4-6` with `branch_depth: 2`, `branch_width: 0.45-0.55`, `branch_intensity: 0.5-0.6` so the tree thins to nothing, `impact_flare: 0.5-0.9` at the ground, and the cross-section `core_width: 0.10-0.16` + `glow_width: 3.0-3.6` - a thin white-hot line inside a broad coloured haze. emissive 14-24; the core washes to white by itself, so keep `color` a saturated electric blue ([0.42,0.62,1,1]) and let the bloom do the white. Secondary arcs: width 0.05-0.09, detail 2, branching 1-2 at depth 1, core_width 0.16-0.2, glow_width 2.6-3, emissive 7-11, no flare. Stage it dark: background near black-blue, ground_albedo 0.08-0.12, grid off, exposure 0.8-0.95, bloom_intensity 0.16-0.24 - a bright stage turns every bolt into a white smear.
- Channel / drain / tether (a sustained stream between two anchors; never draw the characters): the read is DIRECTION, so everything that moves must travel the same way - for a drain, from the victim to the caster. Strands are beams with `origin` = the end the flow comes FROM and `target` = the end it goes TO, `jitter_rate: 0`, `detail: 0`, segments 48-56, `noise_frequency` 0.15-0.3, `noise_amplitude` 0.1-0.5, `noise_scroll` 1.5-3.5 m/s (the undulation itself travels origin -> target), `noise_taper: 0.3` (every strand leaves both anchors smoothly, so several braid into a spindle), `width_profile: taper_both`, and `noise_loop` = the length of the sustain phase so the motion repeats exactly and a game can hold the channel. Layer two kinds: soft "silk bands" (`core_width: 0`, `glow_width: 1`, width 0.18-0.45, emissive 11-13, no pulse) under one or two thin cored filaments (width 0.06-0.09, `core_width: 0.2`, `glow_width: 3`, emissive 1.3-1.6, `pulse_speed` = whole pulses / noise_loop). Around them, per-particle ribbons: a `trail` whose `source` is a particle_system (`render_mode: none`) born at the origin anchor with velocity 9-11 along the flow and spread 9-15, pulled by an `attractor` at the other anchor (strength 80, particle drag 6 -> about 13 m/s) plus curl_noise (strength 11, frequency 1), and HELD on a small sphere collider there (radius 0.17, bounce 0, friction 1 on both) with a lifetime longer than the flight, so each ribbon is drawn into the anchor instead of popping when its head dies. Motes and droplets torn off the victim use the same attractor with a `kill_on_collision` sphere at the hand. Connect and release by keyframing the beams' moving end along the line between the anchors (tools/generators/drain_variants.py). A flare with MANY irregular rays is a swarm of short-lived `stretched_billboard` particles (sphere emitter, `surface_only`, radial direction, velocity 0.9-1.9, velocity_stretch 6, lifetime 0.24, size 0.05-0.09) - never a `star` texture.
...
- Emblems and glyph plates (a shield, a seal, an icon on a buff): the `shape` texture op is an exact signed-distance silhouette - `shape: shield|hexagon|polygon (sides)|diamond|rounded_box|circle`, `radius` = half the height, `aspect`, `center`, `rotation`, `corner_radius`, `softness`, and three modes: `fill`, `outline` (`outline_width`; set `inset` to half of it to stay inside the silhouette) and `bevel` (0 at the contour climbing to 1 at depth `bevel`: `invert` it and multiply by a fill for an inner glow; `inset: -g` with `bevel: g` is an outer glow). A heater-shield emblem, 256x256: `rim` = shape {shape:"shield", radius:0.44, shoulder:0.27, crest:0.06, mode:"outline", outline_width:0.034, inset:0.017, softness:0.006}; a soft copy of it (softness 0.05) at 0.5 for the glow; `line` = the same with outline_width 0.008, inset 0.064 at 0.78; `face` = mode fill, inset 0.076, multiplied by a two-tone split (gradient_linear angle 0 through levels in_low 0.492 in_high 0.508 out_low 0.62 out_high 0.36) and a top-to-bottom falloff (gradient_linear angle 90, start 1, end 0.55); the centre ridge = shape {shape:"rounded_box", radius:0.36, aspect:0.009, softness:0.011} multiplied by the face at 0.8; fold them with math max. Draw it as ONE long-lived billboard particle (emitter rate 0, burst_count 1, burst_times [0]; particle lifetime = how long it shows; size / opacity / emissive over life do the materialise, the pulse and the fade), additive, emissive 1.5, with a soft radial glow sprite twice its size behind it. `shape` is also the only pattern op with a `center`, so it is how small seals (hexagon outlines with a diamond pip) are set into the gaps of a magic circle. Never build a cross or a plus sign out of two boxes.
- Energy bubble / barrier sphere (meshes carry no texture, and a sphere looks the same from every side, so it is camera-facing quads): three single-particle billboards of the same size on the same spot - a fresnel body (gradient_radial linear -> invert -> levels gamma 0.22 out_low 0.1 out_high 0.88, times a `shape` circle, max a thin `shape` circle outline as the crisp silhouette, max a bevel halo outside it), a front shell of cell seams and a dimmer finer back shell rolling the other way (angular_velocity +5 and -4 deg/s). The seams are one `cracks` op (density 5-8, width 0.005-0.008) multiplied by an fbm through levels so the energy varies along them, wrapped onto the sphere with `distort` amount -1 by a packed bulge field: rg = 0.5 + (u-0.5, v-0.5) * g with g = levels(gradient_radial linear radius R, out_high 0.3634, gamma 5) - big cells in the middle, cells squeezed against the rim - then weighted towards the rim. To grow it out of the ground with its base on the floor, spawn the quads at y = 0 with velocity V up and particle drag k: they settle at V * (1 - k/60) / k, and a size_over_life sampled from 1 - (1 - k/60)^(60 * age + 1) keeps radius = height the whole way. Break it up by cross-fading into copies whose material has dissolve 0.96 / erosion 0.05 and a 0.4 s life, plus a burst of tumbling `shard` mesh flakes and sparks from a sphere emitter with surface_only (see tools/generators/shield_variants.py).
