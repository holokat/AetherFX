...
void CpuRuntime::emit_beam(const CompiledNode& cn, const Node& n, double t1) {
    ...
    // Flow (docs/RUNTIME.md section 7): `noise_scroll` slides the displacement
    // field along the beam, so the path undulates like a stream instead of
    ...
    const auto polyline = [&](Vec3 a, Vec3 b, int segs, float amp, double key, uint64_t path_seed) {
        ...
        for (int i = 0; i <= segs; ++i) {
            ...
            if (i > 0 && i < segs && amp > 0.0f) {
                ...
                float reach = amp;
                if (noise_taper > 0.0f)
                    reach *= smoothstep(0.0f, noise_taper, f) * smoothstep(0.0f, noise_taper, 1.0f - f);
                p += u * (reach * n1) + v * (reach * n2);
            }
            points.push_back(p);
        }
