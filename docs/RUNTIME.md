...
* **beam**: origin/target from `origin_node`/`target_node` world positions
  ...
  * **flow** (`noise_scroll` m/s, `noise_loop` s, `noise_taper`): with
    `noise_scroll != 0` the displacement is read at `along - offset` instead of
    `along`, where `along` is the distance from the path's first point and
    `offset = noise_scroll * t1`, so the field travels from the origin to the
    target and the path undulates instead of re-rolling. With `noise_loop = P > 0`
    the phase is `tau = fmod(t1, P)`, `w = tau / P`, and the displacement is
    `((1 - w) * n(along - noise_scroll * tau) + w * n(along - noise_scroll * (tau - P)))
    / sqrt((1 - w)^2 + w^2)`: two copies of one field a period apart, both moving
    at `noise_scroll`, cross-faded so that the path at `t1 + P` equals the path at
    `t1` exactly (at `w = 0` the second term is skipped, at `w -> 1` it has become
    the first), and renormalised because two uncorrelated samples average to a
    smaller one. `noise_taper = k > 0` multiplies the displacement by
    `smoothstep(0, k, f) * smoothstep(0, k, 1 - f)` with `f = i / segments`.
    Branches use the same three terms along their own length. All of it is a pure
    function of `t1`; with the three parameters at 0 the V1 expression is
    evaluated unchanged, bit for bit.
